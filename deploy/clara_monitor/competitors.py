...
def families_for(key: str) -> list[str]:
    """The product families a competitor is actually assigned to compete in.

    Derived from the assignment tables rather than from `segments`, because
    `segments` cannot tell families 2 and 3 apart — both are "haircare" there,
    and a bond-repair house and a hairspray house are not the same rival. The
    assignments already name the exact category, so the family follows from it.

    Ordered by the families' own reading order, so two competitors with the same
    set always list it the same way.
    """
    from . import scope

    found = set()
    for fmt, keys in ASSIGNMENT_BY_FORMAT.items():
        if key in keys:
            # Every format in that table is an appliance.
            found.add("styling_devices")
    for cat, keys in ASSIGNMENT_BY_CATEGORY.items():
        if key in keys:
            fam = scope.family_of("", cat, "")
            if fam != "unclassified":
                found.add(fam)

    # Registered but never assigned: 23 of the 46 are in that position, and
    # filing all of them as unclassified would say the frame does not describe
    # them when the truth is only that no Clara product points at them yet. The
    # declared segments are coarser — "haircare" cannot separate families 2 and
    # 3 — so this resolves to the care family and the assignment tables remain
    # the only thing that can put a competitor in the styling family.
    if not found:
        for seg in REGISTRY[key].segments if key in REGISTRY else ():
            fam = scope.family_of(seg, "", "")
            if fam != "unclassified":
                found.add(fam)
            elif seg == "haircare":
                found.add("hair_scalp_care")
    return [f for f in scope.FAMILY_KEYS if f in found]


def primary_family(key: str) -> str:
    """The one family a competitor is grouped under on a page.

    A rival can compete in several families and most of the big ones do, so a
    page cannot put each card in exactly one place without choosing. The choice
    is the family it is assigned to most often, ties broken by reading order —
    deterministic, so the grouping does not move between runs. The full set is
    still shown on the card, because the primary is a filing decision and not a
    claim that the others do not exist.
    """
    from . import scope
    ...
    for fmt, keys in ASSIGNMENT_BY_FORMAT.items():
        if key in keys:
            counts["styling_devices"] = counts.get("styling_devices", 0) + 1
    for cat, keys in ASSIGNMENT_BY_CATEGORY.items():
        if key in keys:
            fam = scope.family_of("", cat, "")
            if fam != "unclassified":
    ...
    if not counts:
        # No assignments: fall back to the same segment reading `families_for`
        # uses, so a card is never filed somewhere its own family list denies.
        fams = families_for(key)
        return fams[0] if fams else "unclassified"
    order = {f: i for i, f in enumerate(scope.FAMILY_KEYS)}
