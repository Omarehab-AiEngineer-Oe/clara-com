...
The storefront sweep renders as one card per brand; its styling lives in
INTEL_CSS with the rest, not here.
"""
...
/* storefront sweep -- one card per brand.
   These are banner claims, not verified savings, so the card is deliberately
   quieter than .lcard: no price type, no saving colour, nothing that would
   read as a confirmed number. */
.swgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));
        gap:13px;margin-top:15px;align-items:start}
.swbrand{background:var(--card);border:1px solid var(--line);border-radius:8px;
         display:flex;flex-direction:column;overflow:hidden}
.swbrand .sb{display:flex;gap:9px;align-items:baseline;padding:12px 15px;
             background:var(--card2);border-bottom:1px solid var(--line)}
.swbrand .sbname{font-weight:700;font-size:14.5px;flex:1 1 auto;min-width:0;
                 overflow-wrap:anywhere}
.swbrand .sbn{font-size:10.5px;color:var(--ink2);font-weight:600;
              background:var(--card3);border-radius:11px;padding:3px 9px;
              white-space:nowrap;flex:0 0 auto}
.swlines{padding:0 15px;flex:1 1 auto}
.swline{display:flex;gap:10px;align-items:flex-start;padding:10px 0;
        border-top:1px solid var(--line);font-size:12.5px;line-height:1.55}
.swline:first-child{border-top:0}
.swmechs{display:flex;flex-direction:column;gap:3px;flex:0 0 auto;
         width:88px;padding-top:1px}
.swmech{font-size:9px;font-weight:700;letter-spacing:.04em;padding:3px 6px;
        border-radius:3px;background:var(--card3);color:var(--ink2);
        text-transform:uppercase;text-align:center;line-height:1.35}
.swm-percent_off{background:var(--ok-wash);color:var(--ok)}
.swm-free_shipping{background:var(--rival-wash);color:var(--rival)}
.swm-bundle{background:var(--clara-wash);color:var(--clara)}
.swm-free_gift,.swm-gift_with_purchase{background:var(--amb-wash);color:var(--amb)}
.swm-coupon{background:var(--amb-wash);color:var(--amb)}
.swm-clearance,.swm-installments{background:var(--no-wash);color:var(--no)}
.swtext{color:var(--ink);flex:1 1 auto;min-width:0;overflow-wrap:anywhere;
        display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;
        overflow:hidden}
.swopen{font-size:11.5px;color:var(--rival);text-decoration:none;
        white-space:nowrap;flex:0 0 auto;border:1px solid var(--line2);
        border-radius:4px;padding:2px 8px}
.swopen:hover{border-color:var(--rival)}
.swmore{font-size:11.5px;color:var(--ink3);padding:9px 0;
        border-top:1px solid var(--line)}
.swfoot{padding:10px 15px;border-top:1px solid var(--line);
        background:var(--card2);margin-top:auto}
"""
...
def _mechanisms(raw: str, limit: int = 3) -> list[str]:
    """The mechanism field is a comma-separated list, not a single value."""
    seen, out = set(), []
...
def _storefront_offers(bundle: dict) -> str:
    ...
    for brand, offers in sorted(by_brand.items(),
        ...
        P.append(f'<div class="sb"><span class="sbname">{_e(brand)}</span>'
                 f'<span class="sbn">{len(offers)} line(s)</span></div>')
        P.append('<div class="swlines">')
        for o in offers[:5]:
            P.append('<div class="swline">')
            P.append('<span class="swmechs">')
            # a page usually advertises several mechanisms at once; the old
            # markup kept only the first and dropped the rest on the floor
            for mech in _mechanisms(o.get("mechanism")):
                P.append(f'<span class="swmech swm-{_e(mech)}">'
                         f'{_e(mech.replace("_", " "))}</span>')
            P.append('</span>')
            P.append(f'<span class="swtext">{_e(o.get("wording"))}</span>')
            if o.get("url"):
                P.append(f'<a class="swopen" href="{_e(o["url"])}" '
                         f'rel="nofollow noopener">open</a>')
            P.append('</div>')
        if len(offers) > 5:
            P.append(f'<div class="swmore">and {len(offers) - 5} more '
                     f'line(s) on this storefront</div>')
        P.append('</div>')
        P.append(f'<div class="swfoot">'
                 f'{updated(offers[0].get("last_seen_at"), "read")}</div>')
        P.append('</div>')
    P.append('</div>')
